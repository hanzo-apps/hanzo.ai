...
// The install command shown as the one CLI proof-point inside the product
// preview. One demo, no rotation, no step-typing.
const INSTALL_CMD = "curl -fsSL hanzo.sh | bash";
const DEV_CMD = '$ hanzo dev "Build a RAG API for ./docs. Add /chat. Deploy it."';

// Capability strip under the CTAs. Each item DEEP-LINKS to its own product page
// (the header no longer carries a flat Docs link — every product page owns its
// docs + console CTAs). `dynamic:true` items have their count resolved live from
// the platform at mount (see useModelCount); everything else is a stable label.
const STAT_BAND: ReadonlyArray<{ label: string; href: string; dynamic?: "models" }> = [
  ...
  { label: "Vector DB", href: "/vector" },
  { label: "IAM · KMS", href: "/iam" },
  { label: "Self-hostable", href: "/open-source" },
    ...
    <section className="pt-20 pb-8 px-4 md:px-8 lg:px-12">
      ...
      <div className="relative mx-auto w-full max-w-[1400px] min-h-[640px] rounded-2xl border border-border overflow-hidden bg-gradient-to-br from-neutral-950 via-neutral-900 to-neutral-950">
        ...
        <div className="relative z-10 h-full px-4 sm:px-6 md:px-10 lg:px-12 py-10 sm:py-12 lg:py-16">
          <div className="grid lg:grid-cols-[1.05fr_1fr] lg:gap-10 xl:gap-14 items-center min-w-0">
          {/*
          ...
          */}
            ...
            <div className="flex flex-col min-w-0">
              {/* Eyebrow — a distinct, LINKED product badge (no longer echoes the H1). */}
              <motion.div
              ...
              >
                ...
                >
                  ...
                  Zen 5 is live — {modelCount} open models
              ...
              >
                Connect any AI provider, watch usage and cost in one place, and let Hanzo&apos;s native router serve the most optimal model for every request — <span className="text-foreground font-medium">saving up to 90% on compute</span>. Models, agents, vector search, sandboxes, IAM, KMS, and audit logs, in the cloud or self-hosted. The open, 10× more efficient platform for shipping AI.
              </motion.p>
            ...
            >
              ...
              <p className="mt-4 text-sm text-muted-foreground leading-relaxed">
                One console for models, agents, vector search, gateways, IAM, and audit — in the cloud or self-hosted, with a complete audit trail.
              </p>
            </motion.div>
          </div>
        </div>
