...
export const getColorClasses = (color: string, type: 'gradient' | 'border' | 'text', isHovered: boolean) => {
  ...
  const colorMap: Record<string, Record<string, string>> = {
    // Primary brand color
    brand: {
      gradient: isHovered ? 'from-white/30 to-white/10' : 'from-neutral-900/30 to-neutral-800/30',
      border: isHovered ? 'border-border' : 'border-neutral-700/20',
    ...
    secondary: {
      gradient: isHovered ? 'from-white/20 to-white/30' : 'from-neutral-900/30 to-neutral-800/30',
      border: isHovered ? 'border-border' : 'border-neutral-700/20',
